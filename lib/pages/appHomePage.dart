...
class _AppHomePageState extends State<AppHomePage> {
  ...
  var _service = ServiceAvailability();
  SharedPreferences prefs;
  AvailableServices availableServices;
  Future<AvailableServices> servicesFuture;
  ...
  Future initHomePage() async {
    ...
    servicesFuture = _service.getAvailableServices(LatLng(prefs.getDouble('latitude'),prefs.getDouble('longitude')));
    servicesFuture.then((value) {
        ...
        child: SimpleFutureBuilder.simpler(
          future: servicesFuture,
           builder: (AsyncSnapshot<AvailableServices> snapshot) {
            final availableServices = snapshot.data.services;
            return snapshot.data.services.isEmpty ?  Center(
             child: Text("Sorry we could'nt find any service in your area")
             ) : ListView(
               padding: EdgeInsets.fromLTRB(15, 245, 15, 10),
               children: <Widget>[
                 availableServices.isEmpty ? Center(
                     child: Text(
                         "Sorry we could'nt find any services in your area.")
                 ) : SizedBox(),
                 availableServices.contains('Construction Dumpster')
                     ? _buildContainer(
                     title: tr('Construction_Dumpster'),
                     imgPath: "assets/images/dumpster-bg.png",
                     onTap: () =>
                         CustomNavigator.navigateTo(
                             context, DumpsterListing()))
                     : SizedBox(),
                 availableServices.contains('Scaffolding')
                     ? _buildContainer(
                     title: tr('Scaffolding'),
                     imgPath: "assets/images/scaffolding-bg.png",
                     onTap: () =>
                         CustomNavigator.navigateTo(
                             context, ScaffoldingListing()))
                     : SizedBox(),
                 availableServices.contains('Building Material')
                     ? _buildContainer(
                   title: tr('building'),
                   ...
                   onTap: () =>
                     ...
                     : SizedBox(),
                 availableServices.contains('Finishing Material')
                     ? _buildContainer(
                   imgPath: "assets/images/finishing-materials-bg.png",
                   onTap: () {
                     CustomNavigator.navigateTo(
                         context,
                         FinishingMaterialListing());
                   },
                   title: tr('Finishing_Materials'),
                 )
                     : SizedBox(),
                 availableServices.contains('Delivery Vehicle')
                     ? _buildContainer(
